...
# optimize URW model using alternate formulation
{
 ...
 if (meth=="L-BFGS-B")	w<- optim(p0, fn=logL.alt.URW, control=cl, method=meth, lower=c(NA,0), hessian=hess, x=x)
 else					w<- optim(p0, fn=logL.alt.URW, control=cl, method=meth, hessian=hess, x=x)
